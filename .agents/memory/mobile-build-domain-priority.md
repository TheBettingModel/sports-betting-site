...
Existing App Store binaries that were built with the dev domain cannot receive an OTA published for a newer `appVersion` runtime. They can be repaired by publishing a separate OTA using their exact runtime version; a native App Store release then establishes the new runtime for future updates.
