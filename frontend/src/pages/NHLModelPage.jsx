...
import NHLTabs from "../components/NHLTabs";

function NHLModelPage({ marketFilter = "All", title = "NHL Moneyline Model" }) {
    ...
    fetch(`${API_URL}/model/nhl/today`, {
      ...
      .then((data) => {
        if (Array.isArray(data.plays)) {
          ...
          setError(data.error || "Failed to load NHL model.");
        }
      })
      .catch((err) => {
        if (err.name === "AbortError") return;
        console.error("NHL model fetch error:", err);
        setError("Failed to load NHL model.");
      })
  ...
  const renderCard = (play, index, label = null, featured = false) => (
    ...
    >
      ...
      <div style={signalGridStyle}>
        ...
        <div>
          <h4>🏠 Situation</h4>
          <p>Home Court: {play.home_ice_adjustment ?? "N/A"}</p>
  ...
  return (
    <div style={pageStyle}>
      ...
      <p style={subtitleStyle}>
        NHL model powered by market odds, team ratings, home-court adjustment,
      ...
      {error ? (
        ...
        <p>No NHL plays available right now.</p>
      ) : (
        <>
          {topPlay && (
            <section style={{ marginBottom: "45px" }}>
              ...
              {renderCard(topPlay, 0, "Top NHL Play", true)}
            </section>
          )}
