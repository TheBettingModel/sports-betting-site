...
export default function MLBNRFIPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/model/mlb/nrfi/today`, { signal: controller.signal })
      .then((res) => {
  ...
  const filteredPlays = useMemo(() => {
    return [...plays]
      .filter((play) => play.market === "NRFI/YRFI")
  ...
  return (
    <TBMPage className="mlb-v3-page">
      ...
      <TBMGrid columns={4} className="mlb-v3-kpis">
        <TBMMetric label="NRFI/YRFI Plays" value={playCount} />
          ...
          {topPlay && (
            <section className="mlb-v3-section">
              <div className="mlb-v3-section-header">
                <span>Top Play</span>
                <h2>Best MLB NRFI/YRFI Edge</h2>
              </div>
              <MLBPlayCard play={topPlay} label="Top NRFI/YRFI Play" featured />
            </section>
          )}
