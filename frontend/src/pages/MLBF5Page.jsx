...
export default function MLBF5Page() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/model/mlb/f5/today`, { signal: controller.signal })
      .then((res) => {
  ...
  const filteredPlays = useMemo(() => {
    return [...plays]
      .filter((play) => play.market === "First 5")
  ...
  return (
    <TBMPage className="mlb-v3-page">
      ...
      <TBMGrid columns={4} className="mlb-v3-kpis">
        <TBMMetric label="First 5 Plays" value={playCount} />
      ...
      {error ? (
        ...
        <TBMCard className="mlb-v3-empty">No MLB moneyline plays available.</TBMCard>
      ) : (
        <>
          {topPlay && (
            <section className="mlb-v3-section">
              <div className="mlb-v3-section-header">
                <span>Top Play</span>
                <h2>Best MLB First 5 Edge</h2>
              </div>
              <MLBPlayCard play={topPlay} label="Top First 5 Play" featured />
            </section>
          )}
